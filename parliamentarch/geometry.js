...

{
    ...
    const get_row_thickness = (nrows) => (1.0 / (4*nrows - 2));

    function get_rows_from_nrows(nrows, span_angle=_DEFAULT_SPAN_ANGLE) {
        const rv = [];
    ...
    function get_nrows_from_nseats(nseats, span_angle=_DEFAULT_SPAN_ANGLE) {
        let i = 1;
    ...
    const fillingStrategy = {
        DEFAULT: "default",
        EMPTY_INNER: "empty_inner",
        OUTER_PRIORITY: "outer_priority",
    }

    function get_seats_centers(nseats, min_nrows=0, filling_strategy=fillingStrategy.DEFAULT, span_angle=_DEFAULT_SPAN_ANGLE) {
